...
func datatableHandler(db *sql.DB, w http.ResponseWriter, r *http.Request) {
	...
	err := db.QueryRow("SELECT COUNT(id) FROM master_barang").Scan(&recordsTotal)
	if err != nil {
	...
	rows, err := db.Query(querySelect, searchPattern, searchPattern, searchPattern, searchPattern, searchPattern, searchPattern, length, start)
	if err != nil {
		http.Error(w, "Error:", http.StatusInternalServerError)
		log.Println(err)
		return
	...
	err = db.QueryRow(queryCountFilter, searchPattern, searchPattern, searchPattern, searchPattern, searchPattern, searchPattern).Scan(&recordsFiltered)
	if err != nil {
		http.Error(w, "Error:", http.StatusInternalServerError)
		return
	}
...
func main() {
	...
	http.HandleFunc("/api/barang", func(w http.ResponseWriter, r *http.Request) {
        datatableHandler(db, w, r)
    })
